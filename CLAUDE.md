...
```bash
# List shows local + all remote sessions
agentwire list
# LOCAL:
...
agentwire send -s ml@dotdev-pc "run the tests"

# Read output from remote session
agentwire output -s ml@dotdev-pc -n 50

# Kill remote session
agentwire kill -s training@dotdev-pc

# Create remote worktree session
agentwire new -s ml/experiment@gpu-server
```

### Worktree Sessions

When worktrees are enabled (`projects.worktrees.enabled: true`), sessions with `/` in the name create worktrees:

```bash
# Creates worktree at ~/projects/api-worktrees/feature/
agentwire new -s api/feature

# Recreate destroys worktree, pulls latest, creates fresh
agentwire recreate -s api/feature

# Fork creates new worktree from current branch state
...
agentwire list --json
# {"local": [...], "machines": {"dotdev-pc": [...], "gpu-server": [...]}}
```
...
See [Session Name Formats](#session-name-formats) in CLI Commands. Additional conventions:
