...
│       ├── query_audit.py             # Audit log query tool
│       └── test-damage-control.py     # Interactive testing tool
├── logs/
│   └── damage-control/
...
Test commands and paths interactively before running them:
...
Test specific commands programmatically:
...
# Show today's blocked operations
uv run query_audit.py --today

# Show blocks for specific session
uv run query_audit.py --session mission/auth-refactor

# Search for specific pattern
uv run query_audit.py --pattern "rm -rf"

# Show all operations (blocked and allowed)
uv run query_audit.py --all --tail 50
```
...
- Test patterns interactively before deploying: `uv run test-damage-control.py -i`
...
```bash
uv run test-damage-control.py bash Bash "your command here"
...
Run AgentWire-specific pattern tests:

```bash
cd ~/projects/agentwire
pytest tests/hooks/test_agentwire_patterns.py -v
```
...
```bash
# Start AgentWire session
agentwire start test-session
...
# Check audit logs
cd ~/.agentwire/hooks/damage-control
uv run query_audit.py --session test-session
```
...
- [Mission Integration](../missions/damage-control-integration.md) - Full integration mission details
- [Audit Logging](./audit-logging.md) - Detailed audit log format and querying
